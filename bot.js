...
const http = require('http');
    ...
    setInterval(() => {
    http.get(`http://scorpio.glitch.me/`);
    }, 280000);
...
client.on("message" , async msg => {
 ...
 if(afk){
   ...
   .setColor("RANDOM")
   .setDescription(`${client.emojis.get('779071366208159804')} <@${kisi3}> Şu Anda Afk. \n\n:page_facing_up: **Sebep:** ${sebep}`)
...
client.on('message', msg => {
  ...
  switch(message) {
    case "sa":
      ...
      break;
    case "bb":
      msg.reply('**Görüşürüz yine gel**');
      break;
      case "<@793842983007027200>":
        msg.channel.send('Hayırdır ne etiket atıyon la :angry:');
      break;
    case "<@622021445337481222>": 
      let simdikitarih = moment.utc(msg.createdAt).format('DD MM YYYY');
      let user = client.users.get('622021445337481222');
      let userinfo = {};
        ...
        msg.channel.send(`<@622021445337481222> şuan **${userinfo.status}** durumunda ve **${userinfo.od1}** oynuyor.`);
      break;
  };
});
      ...
      try {
        if (!msg.member.hasPermission("MANAGE_WEBHOOKS")) {
          ...
          let embed = new Discord.RichEmbed()
            .setColor(0xffa300)
            .setFooter("ScOrPiO Pro Küfür Sistemi", client.user.avatarURL)
            .setAuthor(
